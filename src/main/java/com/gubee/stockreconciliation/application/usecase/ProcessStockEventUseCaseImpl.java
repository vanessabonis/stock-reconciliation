 ...
 * de lock otimista.
 *
 * Idempotência é garantida pelo ON CONFLICT DO NOTHING no início da transação do processor:
 * duplicatas retornam IGNORED imediatamente, sem executar lógica de negócio e sem lançar exceção.
 *
        ...
        for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                if (attempt == MAX_RETRIES) {
                    ...
                    stockMetrics.recordEventFailed(event.type().name(), "optimistic_lock_exhausted");
                    stockMetrics.recordProcessingTime(timer, event.type().name());
